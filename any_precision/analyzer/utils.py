...
def load_tokenizer(model_str_or_model_or_tokenizer):
    ...
    print("加载分词器, 通过 analyzer.utils.load_tokenizer ")
    if isinstance(model_str_or_model_or_tokenizer, str):
        model_str = model_str_or_model_or_tokenizer
        return AutoTokenizer.from_pretrained(model_str, trust_remote_code=False)# trust_remote_code=True
    elif isinstance(model_str_or_model_or_tokenizer, PreTrainedModel):
        model_str = model_str_or_model_or_tokenizer.name_or_path
        # print("tokenizer加载名称:",model_str)
        return AutoTokenizer.from_pretrained(model_str, trust_remote_code=False)  # , use_fast=False, trust_remote_code=False
    else:
